...
int finish_task_switch_handler(struct kprobe *p, struct pt_regs *regs) {
    ...
    struct task_struct *next = container_of(regs->regs[0], struct task_struct, thread.cpu_context);
...
static int __init cache_kmv3_init(void) {
...
static void __exit cache_kmv3_exit(void) {
